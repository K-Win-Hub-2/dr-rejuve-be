...
const {
  filterFirstPaymentData,
  filterSecondPaymentData,
} = require("../helper/treatmentVoucherHelper");
const {
  AddPointByInput,
...
exports.createTS = async (req, res, next) => {
  ...
  console.log("this is create ts req body", req.body);
  ...
  try {
    ...
    if (tvcCreate === true) {
      ...
      let dataTVC = {
        ...
        extra_points: relatedExtraPoint || 0,
      ...
      if (req.body.payWithPoint) {
        parsedMulti.map(async (treatment) => {
          await createPointHistory({
            ...
            relatedTreatment: treatment.item_id,
            point: treatment.point,
            relatedTreatmentVoucher: treatmentVoucherResult._id,
          });
        });
      }
      ...
      //this is calculated by rule
        ...
        await AddPointByInput(
          req.body.relatedPatient,
          req.body.pay_point_to_customer
        );
        ...
        await createPointHistory({
          type: "point_earned",
          relatedPatient: req.body.relatedPatient,
          point: req.body.pay_point_to_customer,
          relatedTreatmentVoucher: treatmentVoucherResult._id,
          discountPercent: req.body.discountPercent,
          realTotalAmount: req.body.realTotalAmount,
          discountTotalAmount: req.body.totalAmount,
        });
      }
    }
    ...
    if (treatmentVoucherResult) {
      ...
      }).populate("relatedDiscount multiTreatment.item_id payment");
    }
...
exports.filterTreatmentVoucherService = async (data) => {
  ...
  let PointTotal = 0;
  data.Refund = false;
  let treatmentVouchers = await TreatmentVoucher.find(data)
    .populate(
      "newTreatmentVoucherId multiTreatment.item_id medicineItems.item_id relatedTreatment secondAccount relatedDoctor relatedBank relatedCash relatedTreatmentSelection relatedPackage relatedPackageSelection relatedTherapist relatedAccounting payment createdBy salesPerson"
    )
    .populate({
  ...
  let filterOnlySecondAccount = treatmentVouchers.filter(
    (data) => data.secondAccount
  );
