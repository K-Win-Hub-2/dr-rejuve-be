...
const { encryptPassword } = require('../helper/passwordDecryptAndEncryptHelper');
...
exports.listAllPatients = async (req, res) => {
  ...
  try {
    ...
    let result = await Patient.find(query).skip(skip * limit).limit(limit).populate('img').populate({path: "tierLevel", populate: { path: "relatedFixedDiscountPercent"}});
...
exports.getPatient = async (req, res) => {
  ...
  const result = await Patient.find(query).populate('img').populate("tierLevel").populate({
    path: 'relatedMember',
    model: 'Members',
    populate: {
...
exports.updatePatient = async (req, res, next) => {
  ...
  try {
    ...
    if(req.body.password){
       const tierLevelQuery = await tierLevel.find().sort({total_point: 1})
       data.total_point = 5
       data.password = await encryptPassword(req.body.password)
       data.tierLevel = tierLevelQuery[0]._id
       data.walletUser = true
    }
    // if(String(req.body.id)!= String(req.credentials.id)) return res.status(403).send({success: false, message: "You can't edit other user's account."})
