...
const { deductSuccessOrFailure, deductPointFromPatient } = require('../services/pointDeduct');
const { createTS } = require('../services/treatmentVoucherService');
const { AddPointByInput, CalculateAndPointToUserWithPointRule, CalculateTotalAmountByPercent } = require('../helper/pointHelper');
...
exports.createMultiTreatmentSelection = async (req, res) => {
    let { payWithPoint, add_point, input_point, ...data } = req.body
    ...
    if(add_point && add_point === "true"){
        ...
        if(input_point) await AddPointByInput(req.body.relatedPatient, input_point)   
        else if(!input_point) {    
          const percentByAmount = await CalculateTotalAmountByPercent(req,res)
          if(!percentByAmount.success) return res.status(500).send({success: false, message: percentByAmount.message})
        ...
        const total_point = treatmentArray.reduce((acc, curr) => acc + curr.point, 0)
        console.log("d",deductSuccessOrFailure(patientInfo.total_point, total_point))
        switch(deductSuccessOrFailure(patientInfo.total_point, total_point)){
            case false:
                return res.status(401).send({success: false, message:"You don't have enough point to buy these treatments"})
            case true:
...
exports.createTreatmentSelection = async (req, res, next) => {
    ...
    try {
       ...
       let deferRevenues = req.body.totalAmount;
        ...
        let updatedTreatementSelection = await TreatmentSelection.findByIdAndUpdate(populatedResult[0]._id,
              {
                perAppointmentPrice:perAppointmentPrices,
                deferRevenue:deferRevenues
              } 
         )  
        let updatedTreatmentSelectionQuery = await TreatmentSelection.findById(populatedResult[0]._id);
...
exports.updateTreatmentSelectionStatus = async (req, res) => {
    try {
        ...
        const updatePatient = await Patient.findOneAndUpdate({ _id: relatedPatient, isDeleted: false }, { $inc: {  finishedTS: 1, unfinishedTS: -1 } })
