...
async function getPointDiscountRule(data){
    let gift_Rule 
    let { discountId, totalAmount, treatmentType, relatedGiftPointRules, tsType, msTotalAmount } = data
    const { point, amount } = await pointRule.findOne({type: treatmentType})
    relatedGiftPointRules ? gift_Rule = await giftPointRule.findById(relatedGiftPointRules) : gift_Rule = { point: 0 }
    const giftPoint = gift_Rule.point
    if(tsType === "MS"){
        const pointAmount = ( Number(msTotalAmount) * Number(point)/ Number(amount) ) + giftPoint
        return { point: pointAmount, gift_point: giftPoint }
    }
    const { percent } = await fixedDiscountPercent.findById(discountId)
    const pointAmount = ( Number(totalAmount) * Number(point)/ Number(amount) ) + giftPoint
    const discountAmount = Number(totalAmount) - ( Number(totalAmount) * Number(percent) / 100 )
    const realAmount = Number(totalAmount)
    return { point: pointAmount, discountTotalAmount: discountAmount, realTotalAmount: realAmount, percent: percent, gift_point: giftPoint }
}

async function getPointRule(treatmentType) {
    const pointData = await pointRule.find({type: treatmentType}).sort({amount: -1})
    return pointData[0]
}
...
exports.calculateDiscountByTotalAmount = (pointData, total, percent) => {
    ...
    for(let i = 0; i < pointData.length; i++) {
        ...
        if(percent){
            ...
            const percentFromArray = pointRule.filter(po => po === Number(percent))[0] || 0
            return { percent: percentFromArray, point: 1000 }
        }
        else if (firstAmount === secondAmount && totalAmount >= firstAmount){
            return pointData[i]
        }
        else if(firstAmount === totalAmount || (totalAmount > firstAmount && totalAmount < secondAmount)) {
            return pointData[i]
        }
...
exports.CalculateAndPointToUserWithPointRule = async (req,res) => {
    ...
    await patient.findByIdAndUpdate(req.body.relatedPatient,{$inc: { total_point: addPoint }})
    return addPoint
}

exports.CalculateTotalAmountByPercent = async (req,res) => {
    let pointData 
    const { relatedPatient, totalAmount, treatmentType, relatedGiftPointRules, tsType, msTotalAmount } = req.body
    const patientData = await patient.findOne({ _id: relatedPatient, walletUser: true}).populate("tierLevel")
    if(!patientData) return { success: false, message: "Current User is not a wallet user." }
    tsType === "TSMulti" ? pointData = await getPointDiscountRule({discountId: patientData.tierLevel.relatedFixedDiscountPercent, totalAmount: totalAmount, treatmentType: treatmentType, relatedGiftPointRules: relatedGiftPointRules}) : pointData = await getPointDiscountRule({tsType: tsType, treatmentType: treatmentType, relatedGiftPointRules: relatedGiftPointRules, msTotalAmount: msTotalAmount})
